...
TOKEN = '7847048322:AAGY3o9H1rx-s-Q6yKz7aIqLMXlueARDYZo'
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID = -1002427268111          #telegram mai group bana ke chat id dalo      
CHANNEL_ID = -1002427268111                      #telegram mai group bana ke chat id dalo      
error_channel_id = -1002427268111                   #telegram mai group bana ke chat id dalo      
...
def approve_or_disapprove_user(message):
    ...
    if action == '/approve':
        ...
        msg_text = (f"*💫Congratulations💫*\n"
                    ...
                    f"*Welcome to sarfira ddos world✨*")
    else:  # disapprove
        users_collection.update_one(
            {"user_id": target_user_id},
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 JAA PHLE APPROVE HOKE AA APNE PAPA SE *\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @HT_HASEEN*", parse_mode='Markdown')  # Contact owner message
            return
        ...
        bot.send_message(chat_id, "*💣JALDI ATTACK MAAR BAHI KISI NE BHI NI LAGA RAKHA *\n"  # Ready to launch message
                                   ...
                                   "*OWNER @HT_HASEEN🧛*", parse_mode='Markdown')  # Start chaos message
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*🚀PING DEKH NICHE 677 HOGYA !👻*\n\n"  # Attack launched message
                                           f"*TIME ==> {duration} seconds send feedback to @HT_HASEEN🔥*", parse_mode='Markdown')  # Duration message
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @HT_HASEEN* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @HT_HASEEN 🙏"  # Contact owner message
    else:
...
def start_message(message):
    try:
        bot.send_message(message.chat.id, "*🌍 WELCOME TO SARFIRA DDOS WORLD!* 🎉\n\n"
...
if __name__ == "__main__":
    ...
    logging.info("Bot started join @HT_HASEEN...")
    while True:
        try:
